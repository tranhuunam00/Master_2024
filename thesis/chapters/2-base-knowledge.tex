...
qua phân bố trọng lực và các tín hiệu bổ sung khác
...
còn bộ lọc biến thiên toàn phần (total variation filter) được đánh giá cao trong việc bảo toàn biên
chuyển động rõ nét \cite{sun2017sleepmonitor, kalman, Cuadrado2021_kalman, Abbasi_filter}.
...
chuẩn hóa chuỗi thời gian (time-series normalization),
nhằm đảm bảo tính đồng nhất về tần số lấy mẫu giữa các kênh dữ liệu và
duy trì tính toàn vẹn của đặc trưng tín hiệu. Các phương pháp nội suy spline
...
hình học máy hoặc mạng nơ-ron, giúp cải thiện khả năng trích xuất và học
đặc trưng tiềm ẩn của tín hiệu \cite{Lepot2017_interpolation}.
...
cả hai loại tín hiệu - gia tốc (ACC) và xung quang học (PPG) - đều được nội
suy để tạo thành chuỗi thời gian đồng nhất với tần số lấy mẫu 32~Hz.
Đối với các tín hiệu có tần số cao hơn 32~Hz, một bộ lọc thông thấp
(low-pass filter) được áp dụng trước khi giảm tần số lấy mẫu (down-sampling)
nhằm tránh hiện tượng chồng phổ (aliasing).
Ngược lại, với các tín hiệu có tần số lấy mẫu thấp hơn 32~Hz,
dữ liệu được nội suy bằng đa thức Hermite từng đoạn
(Piecewise Cubic Hermite Interpolation Polynomial - PCHIP) để đạt được
tần số mục tiêu, đảm bảo sự đồng nhất và tính ổn định phổ trong toàn bộ
tập dữ liệu.
...
\textbf{Trích xuất đặc trưng} là một bước quan trọng trong quy trình tiền xử lý dữ liệu,
nhằm chuyển đổi tín hiệu cảm biến thô thành tập hợp các đặc trưng có ý nghĩa,
phản ánh những mẫu hình sinh lý liên quan trực tiếp đến mục tiêu nghiên cứu.
...
các đặc trưng được sử dụng phổ biến bao gồm: biến thiên nhịp tim (HRV), khoảng
RR và biên độ sóng R từ tín hiệu ECG; các chỉ số thống kê, tần số và entropy từ
tín hiệu $\mathrm{SpO_2}$; hoặc các đặc trưng phi tuyến và miền tần số như năng
lượng phổ EEG. Ngoài ra, các hệ số wavelet thu được từ tín hiệu gốc có khả năng
phản ánh rõ ràng chu kỳ hô hấp, chuyển động cơ thể, cũng như các đặc điểm bất
thường liên quan đến các giai đoạn ngưng thở. Nhờ đó, việc trích xuất đặc trưng
wavelet đóng vai trò quan trọng trong việc nâng cao hiệu suất của các mô hình
phân loại tư thế ngủ và phát hiện OSA \cite{RAJESH2021104199_wavelet}.

Đối với bài toán phân loại tư thế ngủ sử dụng cảm biến gia tốc đơn,
các đặc trưng thường được trích xuất trực tiếp từ miền thời gian của tín hiệu 3 trục thô.
...
bao gồm giá trị trung bình (mean) và trung vị (median) của ba trục gia tốc (X, Y, Z)
cùng với các góc nghiêng giữa các mặt phẳng XY, YZ và XZ, tạo thành tổng cộng 12
đặc trưng cho mỗi cửa sổ dữ liệu.
Trong khi đó, nghiên cứu \cite{Vu2025SleepPosition, vu2023} không trích xuất đặc trưng thủ công (như
mean, median hoặc wavelet), mà đưa trực tiếp tín hiệu gia tốc thô $(a_x, a_y,
  a_z)$ vào mô hình LSTM. Mạng học sâu này tự động học các đặc trưng không
gian-thời gian trong chuỗi 20 mẫu, nhờ đó mô hình đạt độ chính xác cao mà không
cần bước trích chọn đặc trưng trung gian.
...
tư thế ngủ một cách hiệu quả và khả thi khi triển khai trên thiết bị biên (edge
devices). Do đó, từ phần tiếp theo, nội dung nghiên cứu sẽ tập trung vào quy
trình nghiên cứ, huấn luyện và đánh giá hiệu năng của mô hình học máy cho bài
toán phân loại tư thế ngủ.

Các kỹ thuật học máy (Machine Learning - ML) và kiến trúc mạng nơ-ron nhân tạo
(Artificial Neural Networks - ANN) đã được ứng dụng rộng rãi và cho thấy hiệu
quả cao trong các bài toán phân loại tư thế ngủ.
...
như ngưỡng định sãn hoặc mô hình học máy truyền thống như KNN, SVM và RF. Tuy
nhiên, một điểm đáng lưu ý là chỉ duy nhất nghiên cứu của Abdulsadig và cộng sự
...
Đi sâu hơn, nhóm mô hình học sâu LSTM hoặc kết hợp nhiều lớp tuy đạt độ chính xác vượt trội (trên 99\%) như trong nghiên cứu của Vu et al. \cite{vu2023}, nhưng đều chưa tích hợp trực tiếp lên vi điều khiển do dung lượng bộ nhớ lớn và nhu cầu tính toán cao. Điều này đặt ra thách thức cho việc ứng dụng chúng trong môi trường thực tế như giấc ngủ tại nhà, nơi mà năng lượng, khả năng lưu trữ và kết nối đều bị hạn chế.
...
không cần FFT - giúp giảm đáng kể độ phức tạp tính toán \cite{gomes2021}, \cite{souza2017}.
...
Vì vậy, luận văn được tổ chức theo hai giai đoạn mạch lạc: (i) \emph{Đánh giá
  đặc trưng và mô hình trên dữ liệu thu thập} - xây dựng chuỗi xử lý từ tiền xử
lý, phân đoạn theo cửa sổ, trích xuất đặc trưng đại diện (miền thời gian và tần
số) đến so sánh các mô hình nhẹ (ngưỡng, LR, SVM, RF, NN ). Tiêu chí lựa chọn
không chỉ dựa trên độ chính xác/F1 và ma trận nhầm lẫn cho từng tư thế, mà còn
cân nhắc tính bền vững trước sai lệch gắn cảm biến, kích thước mô hình, chi phí
tính toán và khả năng giảm tần số lấy mẫu. (ii) \emph{Triển khai on-chip
  (TinyML) trên vi điều khiển mục tiêu} - nền tảng Cortex-M4 bảo đảm ràng buộc bộ
nhớ-độ trễ-năng lượng, đồng thời kiểm chứng sai lệch hiệu năng so với giai đoạn
(i) ở mức chấp nhận được cho ứng dụng thực tế.
